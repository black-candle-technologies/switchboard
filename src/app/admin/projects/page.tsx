...
type ListRow = Prisma.ProjectGetPayload<{
...
export default async function ProjectListPage({ searchParams }: PageProps) {
  ...
  const safeRequestedPage =
    Number.isSafeInteger(requestedPage) && requestedPage > 0
      ? requestedPage
      : 1;
  const configuredTake = ProjectResource.list?.perPage ?? 20;
  const take = Math.min(100, Math.max(1, Math.trunc(configuredTake)));

  const supportedSearchFields = new Set(["name", "description"]);
  const searchable = (
    ProjectResource.list?.searchable ?? ["name", "description"]
  ...
  const configuredDefaultSort = ProjectResource.list?.defaultSort;
  ...
  const items: ListRow[] = await prisma.project.findMany({
    where,
    orderBy,
    skip: (page - 1) * take,
    take,
  ...
  const columns = (ProjectResource.list?.columns ?? [
    {
      key: "id",
      header: "Id",
    },
    ...
    <section className="sb-page">
      ...
      {searchable.length ? (
        <form method="get" className="sb-search-form">
          ...
          {q ? (
            <Link className="sb-action-link" href="/admin/projects">
              Clear
            </Link>
          ) : null}
        </form>
            ...
            {items.length === 0 ? (
              <tr>
                <td className="sb-empty-state" colSpan={columns.length + 1}>
                  {q
                    ? `No project records match "${q}".`
                    : "No project records yet. Create one to get started."}
                </td>
              </tr>
            ) : null}
          </tbody>
