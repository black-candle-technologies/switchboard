...
export default async function ProjectListPage({ searchParams }: PageProps) {
  ...
  const generatedColumns = (ProjectResource.list?.columns ?? [
    {
      key: "name",
      header: "Name",
    },
  ...
  const columns: Column<Project>[] = generatedColumns.map((column) => {
    const baseColumn: Column<Project> = {
      key: column.key,
    ...
    <section className="sb-page">
      <div className="sb-page-header">
        <div>
          ...
          <h1 className="sb-page-title">Projects</h1>
          <p className="sb-page-description">
            View, search, and manage project records.
          </p>
        </div>
        <Link href="/admin/projects/new" className="sb-button">
          + New
        </Link>
      </div>
