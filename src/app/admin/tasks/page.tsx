...
type ListRow = Prisma.TaskGetPayload<{
...
export default async function TaskListPage({ searchParams }: PageProps) {
  ...
  const safeRequestedPage =
    Number.isSafeInteger(requestedPage) && requestedPage > 0
      ? requestedPage
      : 1;
  const configuredTake = TaskResource.list?.perPage ?? 20;
  const take = Math.min(100, Math.max(1, Math.trunc(configuredTake)));

  const supportedSearchFields = new Set(["title"]);
  const searchable = (TaskResource.list?.searchable ?? ["title"]).filter(
    (field) => supportedSearchFields.has(field),
  );
  ...
  const configuredDefaultSort = TaskResource.list?.defaultSort;
  ...
  const items: ListRow[] = await prisma.task.findMany({
    where,
    orderBy,
    skip: (page - 1) * take,
    take,
  ...
  const columns = (TaskResource.list?.columns ?? [
    {
      key: "id",
      header: "Id",
    },
    ...
    <section className="sb-page">
      ...
      {searchable.length ? (
        <form method="get" className="sb-search-form">
          ...
          {q ? (
            <Link className="sb-action-link" href="/admin/tasks">
              Clear
            </Link>
          ) : null}
        </form>
            ...
            {items.length === 0 ? (
              <tr>
                <td className="sb-empty-state" colSpan={columns.length + 1}>
                  {q
                    ? `No task records match "${q}".`
                    : "No task records yet. Create one to get started."}
                </td>
              </tr>
            ) : null}
          </tbody>
